...
| `novaview_datalogger.exe.spec` | PyInstaller build spec.                                       |
...
| `img/`                        | `novaview_datalogger_logo.png` (runtime logo) and `icon_transparent.png` (source for the `.exe` icon). |
...
`*.db`, `img/icon.ico`) is generated and is ignored by git.
...
The script installs dependencies, regenerates `img/icon.ico` from
`img/icon_transparent.png`, clears any previous `build/` and `dist/`, then runs
PyInstaller against `novaview_datalogger.exe.spec`.

Output: `dist/novaview_datalogger.exe` (single-file, no console window).
